...
fasta.obj(queryFilename).on ('data', function (data) {
  ...
  console.log (matches + " matches, " + (mismatches + " " + "mismatches").bgCyan.blue + ", " + (insertions + " " + "insertions").green.inverse + ", " + (deletions + " " + "deletions").bgRed.white)
...
function prettyPrint (alignment, labels, refRowIndex) {
  ...
  for (var colStart = 0; colStart < alignment.length; colStart += alignCols) {
	...
	process.stdout.write (alignChar === refChar ? alignChar : (alignChar === '-' ? alignChar.bgRed.white : (refChar === '-' ? alignChar.green.inverse : alignChar.bgCyan.blue)))
      }
